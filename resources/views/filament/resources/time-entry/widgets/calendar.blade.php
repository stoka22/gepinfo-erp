            ...
            <div class="grid grid-cols-7 gap-px rounded-xl overflow-hidden bg-white/10">
                @foreach($this->days as $day)
                    @php
                        ...
                        // ha ma van és van bejegyzés → erősebb highlight
                        $today  = $isToday && $has ? 'bg-emerald-600/15 ring-1 ring-emerald-400/30' : '';

                        // hónapon kívüli napok halványabbak
                    ...
                    >
                        ...
                        <div class="flex items-center justify-between text-xs text-white/70">
                            ...
                            @if($has)
                                <span
                                    class="inline-flex items-center justify-center w-5 h-5 text-[10px] rounded-full bg-white/10 border border-white/20">
                                    {{ count($day['entries']) }}
                                </span>
                            @endif
                        </div>
                        ...
                        {{-- teljes cella kattintható, ha van adat --}}
                        @if($has)
                            ...
                            <tbody>
                                <template x-for="it in items" :key="it.id">
                                    <tr class="border-t border-white/10">
                                        ...
                                        <td class="p-3">
                                            <span
                                                x-text="it.type === 'vacation' ? 'Szabadság' : (it.type === 'overtime' ? 'Túlóra' : 'Táppénz')">
                                            </span>
                                        </td>
                                        <td class="p-3">
                                        ...
                                        <td class="p-3">
                                            <span
                                                ...
                                                :class="{
                                                    'bg-white/10': true,
                                                    'text-emerald-200': it.status === 'approved',
                                                    'text-rose-200': it.status === 'rejected',
                                                    'text-white/70': it.status === 'pending',
                                                }"
                                                x-text="it.status === 'approved' ? 'Jóváhagyva' : (it.status === 'rejected' ? 'Elutasítva' : 'Függőben')">
                                            </span>
                                        </td>
                                    </tr>
                                </template>
